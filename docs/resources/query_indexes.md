...
- `index_keys` (List of String) The index key expressions. Write each key in the canonical form Couchbase returns: backtick-quote identifiers and keep any key modifiers, e.g. "`geo`", "`age` INCLUDE MISSING", "`vec` VECTOR". Changing index_keys forces the index to be recreated, so matching the canonical form avoids a spurious replacement when the index is imported.
