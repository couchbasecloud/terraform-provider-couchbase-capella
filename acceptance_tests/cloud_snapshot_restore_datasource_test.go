...
func TestAccDatasourceCloudSnapshotRestores(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccCloudSnapshotRestoreBackupOnlyConfig(backupResourceName),
			},
			// Step 2: trigger a restore by setting restore_times, then read the list.
			{
				Config: testAccCloudSnapshotRestoresDatasourceConfig(backupResourceName, dsName),
...
func TestAccDatasourceCloudSnapshotRestore(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccCloudSnapshotRestoreSingleDatasourceConfig(backupResourceName, listDsName, singleDsName),
				Check: resource.ComposeAggregateTestCheckFunc(
...
func testAccCloudSnapshotRestoreBackupOnlyConfig(backupResourceName string) string {
	return fmt.Sprintf(`
%[1]s
