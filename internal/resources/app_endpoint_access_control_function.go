...
func (a *AccessControlFunction) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		if isForbiddenError(err) {
			result, msg := checkAppEndpointDeletedOrForbidden(ctx, a.Data, IDs["organizationId"], IDs["projectId"], IDs["clusterId"], IDs["appServiceId"], IDs["appEndpointName"])
			switch result {
			...
			case appEndpointExists:
				resp.Diagnostics.AddError("Error Reading Capella Access Function", msg)
				return
			default:
				resp.Diagnostics.AddError("Error Reading Capella Access Function", msg)
				return
