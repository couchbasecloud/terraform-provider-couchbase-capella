...
const (
	...
	errorMessageWhileLogStreamingCreation = "There is an error during log streaming configuration. Please check in Capella to see if any hanging resources" +
...
func (r *AppServiceLogStreaming) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err := providerschema.ValidateAppServiceLogStreamingRequest(plan); err != nil {
		resp.Diagnostics.AddError(
			...
			"Could not create log streaming configuration: "+err.Error(),
		)
		return
	}
	...
	plan.Id = types.StringValue(appServiceId)
	plan.ConfigState = types.StringValue(string(apigen.GetLogStreamingResponseConfigStateEnabling))
	...
	if err != nil {
		resp.Diagnostics.AddWarning(
			...
			errorMessageAfterLogStreamingCreation+api.ParseError(err),
		)
		return
	}
...
func (r *AppServiceLogStreaming) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(
			...
			"Could not read log streaming configuration: "+err.Error(),
		)
		return
	}
	...
	if err != nil {
		...
		resp.Diagnostics.AddError(
			"Error reading log streaming configuration",
			"Could not read log streaming configuration for app service "+appServiceId+": "+errString,
		)
		return
	}
...
func (r *AppServiceLogStreaming) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError(
			"Error updating log streaming configuration",
			"Could not update log streaming configuration for app service "+appServiceId+": "+err.Error(),
		)
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			...
			"Could not update log streaming configuration for app service "+appServiceId+": "+api.ParseError(err),
		)
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError(
			...
			"Could not refresh log streaming configuration for app service "+appServiceId+": "+api.ParseError(err),
		)
		return
	}
...
func (r *AppServiceLogStreaming) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError(
			"Error deleting log streaming configuration",
			"Could not delete log streaming configuration for app service "+appServiceId+": "+errString,
		)
		return
	}

	// Handle 404 gracefully - resource already deleted
	...
	if err != nil {
		// Check if it's a 404, which means deletion completed
		resourceNotFound, errString := api.CheckResourceNotFoundError(err)
		if !resourceNotFound {
			resp.Diagnostics.AddError(
		...
		// 404 means resource was successfully deleted
	}
}
...
func (r *AppServiceLogStreaming) refreshLogStreaming(
	...
	if response.StatusCode() == http.StatusNotFound {
		return nil, fmt.Errorf("%s: %w", errors.ErrNotFound, fmt.Errorf("log streaming configuration not found for app service %s", appServiceId))
	}
...
func (r *AppServiceLogStreaming) waitForLogStreamingState(
	...
	for {
		...
		case <-timer.C:
			...
			// Handle 404 - if we're waiting for disabled state, this is acceptable
			if response.StatusCode() == http.StatusNotFound {
				if targetState == apigen.GetLogStreamingResponseConfigStateDisabled {
					return nil
				}
				return fmt.Errorf("log streaming configuration not found")
			}
			...
			if response.JSON200 == nil || response.JSON200.ConfigState == nil {
				tflog.Info(ctx, "waiting for log streaming config state to be available")
			...
			if currentState == string(apigen.GetLogStreamingResponseConfigStateErrored) {
				return fmt.Errorf("log streaming configuration entered errored state")
			}
