...
func BackupSchema() schema.Schema {
	return schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"status":          WithDescription(stringAttribute([]string{computed, useStateForUnknown}), "The status of the backup. Backup statuses are 'pending', 'ready', and 'failed'."),
			...
			"cloud_provider":  WithDescription(stringAttribute([]string{computed, useStateForUnknown}), "The Cloud Service Provider where the cluster is hosted."),
			...
			"restore": schema.SingleNestedAttribute{
				...
				Attributes: map[string]schema.Attribute{
					...
					"force_updates":           WithDescription(boolAttribute(optional), " Forces data in the operational cluster to be overwritten even if the data in the cluster is newer."),
					...
					"replace_ttl":             WithDescription(stringAttribute([]string{optional}), "Sets a new expiration (time-to-live) value for the specified keys. These values are 'none', 'all', and 'expired'."),
