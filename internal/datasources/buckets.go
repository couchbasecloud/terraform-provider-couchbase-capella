...
var (
	_ datasource.DataSource              = &Bucket{}
	_ datasource.DataSourceWithConfigure = &Bucket{}
)

// Bucket is the bucket data source implementation.
type Bucket struct {
	*providerschema.Data
}

// NewBucket is a helper function to simplify the provider implementation.
func NewBucket() datasource.DataSource {
	return &Bucket{}
}
...
func (d *Bucket) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	bucketResp := api.GetBucketsResponse{}
	...
	for _, bucket := range bucketResp.Data {
		bucketState := providerschema.OneBucket{
			...
			Audit: providerschema.CouchbaseAuditData{
				CreatedAt:  types.StringValue(bucket.Audit.CreatedAt.String()),
				CreatedBy:  types.StringValue(bucket.Audit.CreatedBy),
				ModifiedAt: types.StringValue(bucket.Audit.ModifiedAt.String()),
				ModifiedBy: types.StringValue(bucket.Audit.ModifiedBy),
				Version:    types.Int64Value(int64(bucket.Audit.Version)),
			},
