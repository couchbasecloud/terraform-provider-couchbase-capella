...
func GetPaginated[DataSchema ~[]T, T any](ctx context.Context, client *Client, token string, cfg EndpointCfg) (DataSchema, error) {
	var (
	...
	for {
		cfg.Url = baseUrl + fmt.Sprintf("?page=%d&perPage=%d&sortBy=id", page, perPage)
